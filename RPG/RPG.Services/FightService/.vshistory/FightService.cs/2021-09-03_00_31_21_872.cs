...
namespace RPG.Services.FightService
{
    public class FightService : IFightService
    {
        ...
        private int doWeaponAttack(Character attacker, Character oponnent)
        {
        ...
        private int doSkillAttack(Character attacker, Character oponnent, Skill skill)
        {
        ...
        public async Task<ServiceResponse<AttackResultDto>> WeaponAttack(WeapoAttackDto weapoAttackDto)
        {
            ...
            int damage = doWeaponAttack(attacker, oponnent);
        ...
        public async Task<ServiceResponse<AttackResultDto>> SkillAttack(SkillAttackDto skillAttackDto)
        {
            ...
            int damage = doSkillAttack(attacker, oponnent, skill);
            if (oponnent.HitPoints <= 0) { responce.Message = $"Character {oponnent.Name} was defeated"; }
            await _dataContext.SaveChangesAsync();
            responce.Data = new AttackResultDto
            {
        ...
        public async Task<ServiceResponse<FightResultDto>> FightAttack(FightRequistDto fightRequistDto)
        {
            ...
            while (!defeted)
            {
                foreach (var attacker in characters)
                {
                    ...
                    {
                        if (attacker.Skills.Count == 0) { continue; }
                        var skill = attacker.Skills[RandomNumber(attacker.Skills.Count)];
                        attackUsed = skill.Name;
                        damage = doSkillAttack(attacker, oponnent, skill);
                    }

                    responce.Data.Log.Add($"{attacker.Name} Attacks {oponnent.Name} Using {attackUsed} With {(damage >= 0 ? damage : 0)} damage.");
                    if (oponnent.HitPoints <= 0)
                    {
                        defeted = true;
                        attacker.Victories++;
                        oponnent.Defeats++;
                        responce.Data.Log.Add($"{oponnent.Name} has been defeated.");
        ...
        public async Task<ServiceResponse<List<HighScoreDato>>> GetHighScore()
        {
            var characters = await _dataContext.Character
                .Where(c => c.Fights > 0)
                ...
                .ThenBy(c => c.Defeats)
                .ToListAsync();
            var a = _mapper.Map<List<HighScoreDato>>(characters);

            return new() ServiceResponse<List<HighScoreDato>> response
            {
                Data = a
            };
        }
