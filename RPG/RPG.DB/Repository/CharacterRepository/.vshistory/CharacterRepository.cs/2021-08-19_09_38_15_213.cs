...
namespace RPG.Data.Repository.CharacterRepository
{
    public class CharacterRepository : ICharacterRepository
    {
        ...
        public async Task<List<GetCharacterDto>> AddCharacter(AddCharacterDto newCharacter, int userId)
        {
            ...
            await SaveChanges();
            return _mapper.Map<List<GetCharacterDto>>(await _dataContext.Character.Where(c => c.User.Id ==).ToListAsync());
        }
        ...
        {
            var a = await _dataContext.Character.Where(c => c.User.Id == userId).ToListAsync();
        ...
        {
            if (await _dataContext.Character.FirstOrDefaultAsync(c => c.Id == updateCharacter.Id) == null)
            {
                return null;
            }
